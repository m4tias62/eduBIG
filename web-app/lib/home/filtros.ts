...
const DEP_A_COD: Record<Dependencia, number> = {
  publica: 6,
  part_subvencionado: 3,
  part_pagado: 4,
};
...
export function pasaFiltros(c: Colegio, f: FiltrosHome): boolean {
  ...
  if (f.dependencia && c.COD_DEPE !== DEP_A_COD[f.dependencia]) return false;
...
export function parseFiltros(
  ...
  const dep = get("dep");
  if (dep === "publica" || dep === "part_subvencionado" || dep === "part_pagado") f.dependencia = dep;
  return f;
}
...
export function serializeFiltros(f: FiltrosHome, extras: Record<string, string> = {}): string {
  ...
  if (f.dependencia) p.set("dep", f.dependencia);
